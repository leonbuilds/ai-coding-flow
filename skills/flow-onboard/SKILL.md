...
3. **写仓库规则文件**：
   ...
   - 素材是 `.ai/kb/architecture/tech-stack.md` 和 `dev-guide.md`：技术栈版本直接引用；构建、测试命令只抄标了「已验证」的；目录地图只写知识库没覆盖的注意事项，其余一句「详见 `.ai/kb/README.md`」。
   - 「代码知识库」一节写明：先读 `.ai/kb/ai-quick-reference.md`，再按需读页；页里的 `file:line` 仍要打开核对。
...
4. **起草 3–8 条 spec**，放到 `.ai/specs/<类型>/`：
   ...
   优先写这几类：分层依赖规则（素材：`.ai/kb/architecture/module-dependencies.md`）、错误处理约定（素材：`architecture/patterns.md`）、测试写法（素材：`architecture/dev-guide.md`）、领域里容易混淆的概念（素材：`architecture/domain-concepts.md`、各域的 `术语梳理.md`）。spec 是「应该怎样」的规则，知识库是「现在是怎样」的事实，不要把事实抄成 spec。
