...
4. **起草 3–8 条 spec**，放到 `.ai/specs/<类型>/`：
   ...
   优先写这几类：分层依赖规则（素材：`.ai/kb/architecture.md` 的「分层与依赖方向」和「循环依赖」）、错误处理约定（素材：「关键横切机制」）、测试写法、领域里容易混淆的概念（素材：`.ai/kb/overview.md` 的「术语」一节）。spec 是「应该怎样」的规则，知识库是「现在是怎样」的事实，不要把事实抄成 spec。
