...
2. **规划**：`flowctl kb plan`。它按扫描结果给出页面清单：`overview`、`architecture`、`interfaces`、`data-model`、`glossary` 各一页，每个模块一页，写入 `.ai/kb/plan.json`。
...
5. **写作**：按下面的顺序写，模块页先写，因为其他页要引用它们：
   1. 每个 `modules/*.md`；
   2. `interfaces.md`、`data-model.md`、`architecture.md`、`overview.md`；
   3. `glossary.md`。
